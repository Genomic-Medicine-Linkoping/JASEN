...
`r if(params$cgmlst) {"# Cgmlst Stats"}`

```{r eval=params$cgmlst, echo=FALSE, message=FALSE, warning=FALSE}
...
`r if(params$cgmlst) {"# Cgmlst Alleles"}`
...
`r if(params$cgmlst) {"Reference to chewBBACA: @Silva2018"}`

# AMRFinderPlus resistance gene results

```{r echo=FALSE, message=FALSE}
read_tsv("aMRFinderPlus.tsv") %>%
    select(-one_of(c("Protein identifier"))) %>%
    datatable( 
    rownames = FALSE,
    extensions = c('FixedColumns','Buttons','KeyTable'),
    options=list(
      scrollX=TRUE,
      dom = 'tBlrpfi',
      lengthMenu = c(5, 10, 30, 100),
      buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
      keys = TRUE,
      pageLength = 5
      ), caption = htmltools::tags$caption(
    style = 'caption-side: bottom; text-align: center;',
    'Table 3: AMRFinderPlus results. For more information on them, see: ', htmltools::tags$a(href="https://github.com/ncbi/amr/wiki/Interpreting-results", "AMR - Interpreting results"), '.')
    )
```

Reference to AMRFinderPlus: @Feldgarden2019
...
read_tsv('mlst_report.tsv') %>%
  datatable(
    ...
    style = 'caption-side: bottom; text-align: center;',
    'Table 6: For more information on Ariba MLST calling, see: ', htmltools::tags$a(href="https://github.com/sanger-pathogens/ariba/wiki/MLST-calling-with-ARIBA", "ariba MLST calling wikipage"))
    )
```

# MLST calling with `tseemann/mlst`
...
k3 %>%
  datatable(
    ...
    style = 'caption-side: bottom; text-align: center;',
    'Table 7: For more information on this calling program, see: ', htmltools::tags$a(href="https://github.com/tseemann/mlst", "mlst Github repository"))
    )
```
...
t(k7d) %>%
  datatable(
    ...
    style = 'caption-side: bottom; text-align: center;',
    'Table 8: Output of genome assembly quality assessment tool. For detailed information on the table headers, see: ', htmltools::tags$a(href="http://quast.sourceforge.net/docs/manual.html#sec3.1.1", "QUAST manual"))
    )
```

Quast html report can be found [here](quast/report.html). 
...
if (dim(k8d)[1] != 0 & dim(k8d)[2] != 0) {
  k8d %>% 
    dt_alise('Table 8: Ariba run output for resistance genes from Resfinder. For detailed information on the table headers, see: ', htmltools::tags$a(href="https://github.com/sanger-pathogens/ariba/wiki/Task:-run#report-file", "ariba run subcommand output description"))
}else{
  # If not found, print out just a plain and simple text
...
MultiQC report can be found [here](multiqc/multiqc_report.html). Reference to MultiQC: @Ewels2016.

`r if(params$spa_exist) {"# Spa type"}`

```{r eval=params$spa_exist, echo=FALSE, message=FALSE}
read_tsv(params$spa_path) %>% 
  datatable(
  rownames = FALSE,
  extensions = c('FixedColumns','Buttons','KeyTable'),
  options=list(
  ...
  style = 'caption-side: bottom; text-align: center;',
  'Table 9: Output of spaTyper tool: ', htmltools::tags$a(href="https://github.com/HCGB-IGTP/spaTyper", "https://github.com/HCGB-IGTP/spaTyper"))
)
```

`r if(params$spa_exist) {"Reference to spaTyper: @jose_francisco_sanchez_herrero_2020_4063625"}`
